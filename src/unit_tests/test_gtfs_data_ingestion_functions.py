...
def test_push_feed_data_to_db():
    # Mock task_instance to return predefined feed data
    task_instance = mock.Mock()
    task_instance.xcom_pull.return_value = [
        [
            {"trip_id": "123", "departure_date": "20230101", "departure_time": "12:00:00"}
        ],
        [
            {"trip_id": "123", "stop_id": "stop1", "arrival_time": 1672531200, "departure_time": 1672531800, "delay_arrival": 60, "delay_departure": 120, "update_time": 1672531200}
        ]
    ]
    
    # Mock connect_to_postgres to return a mock connection
    mock_conn = mock.Mock()
    mock_cursor = mock.Mock()
    mock_conn.cursor.return_value = mock_cursor
    
    with mock.patch('src.airflow.dags.gtfs_data_ingestion_functions.connect_to_postgres', return_value=mock_conn):
        # Test for trips_gtfs_rt table
        ...
        # Test for stop_time_update_gtfs_rt table
        push_feed_data_to_db(task_instance=task_instance, table='stop_time_update_gtfs_rt')
